...
def describe_tasks(role=None):
    ...
    # Handle pagination for clusters
    paginator = client.get_paginator('list_clusters')
    for cluster_page in paginator.paginate():
        for cluster in cluster_page["clusterArns"]:
            # Handle pagination for tasks
            tasks_paginator = client.get_paginator('list_tasks')
            for task_page in tasks_paginator.paginate(
                cluster=cluster,
                ...
                # describe_tasks API has a limit of 100 tasks per call
                for task_arns_chunk in [task_page["taskArns"][i:i+100] for i in range(0, len(task_page["taskArns"]), 100)]:
